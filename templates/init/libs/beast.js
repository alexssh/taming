                ...
                for (var events in decl.on) {
                    if (events === 'preventable') {
                        for (var preventableEvents in decl.on.preventable) {
                            this.on(preventableEvents, decl.on.preventable[preventableEvents], false, decl, false, true)
                        }
                    } else {
                ...
                for (var events in decl.onWin) {
                    if (events === 'preventable') {
                        for (var preventableEvents in decl.onWin.preventable) {
                            this.onWin(preventableEvents, decl.onWin.preventable[preventableEvents], false, decl, false, true)
                        }
                    } else {
...
BemNode.prototype = {
    ...
    on: function (event, handler, isSingleEvent, fromDecl, dontCache, preventable) {
        ...
        if (handler.isBoundToNode === undefined) {
            ...
            handler.preventable = preventable
        }
        ...
        if (!isSingleEvent && event.indexOf(' ') > -1) {
            ...
            for (var i = 0, ii = events.length; i < ii; i++) {
                this.on(events[i], handler, true, fromDecl, dontCache, preventable)
            }
        } else {
            if (this._domNode !== undefined) {
    ...
    onWin: function (event, handler, isSingleEvent, fromDecl, dontCache, preventable) {
        ...
        if (!isSingleEvent && event.indexOf(' ') > -1) {
            var events = event.split(' ')
            for (var i = 0, ii = events.length; i < ii; i++) {
                this.onWin(events[i], handler, true, fromDecl, dontCache, preventable)
            }
        } else {
            if (this._domNode !== undefined) {
        ...
        if (!this._renderedOnce) {
            ...
            if (this._domNodeEventHandlers !== undefined) {
                for (var eventName in this._domNodeEventHandlers) {
                    for (var i = 0, ii = this._domNodeEventHandlers[eventName].length, h; i < ii; i++) {
                        h = this._domNodeEventHandlers[eventName][i]
                        this.on(eventName, h, true, false, true, h.preventable)
                    }
                }
            }
            if (this._windowEventHandlers !== undefined) {
                for (var eventName in this._windowEventHandlers) {
                    for (var i = 0, ii = this._windowEventHandlers[eventName].length, h; i < ii; i++) {
                        h = this._windowEventHandlers[eventName][i]
                        this.onWin(eventName, h, true, false, true, h.preventable)
                    }
                }
            }

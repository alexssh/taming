...
try {
    projectConfig = require(path.resolve(process.cwd(), 'taming.json'))
}
catch(e) {
    projectConfig = {}
}
...
for (let item in config.source) {
    if (projectConfig.source && projectConfig.source.hasOwnProperty(item)) {
        config.source[item] = projectConfig.source[item]
    }
}
...
module.exports = {
    ...
    update: (cb) => {
        fs.readFile(config.templates.taming, 'utf8', function(err, packageJSON) {
            fs.readFile(`./taming.json`, 'utf8', function(err, projectJSON) {
                packageJSON = JSON.parse(packageJSON)
                projectJSON = JSON.parse(projectJSON)

                projectJSON.version = packageJSON.version

                for (key in packageJSON) {
                    if (!(projectJSON[key])){
                        projectJSON[key] = packageJSON[key]
                    }
                }

                gulp.src('./taming.json', {base: './'})
                    .pipe(plumber())
                    .pipe(replace(/[\s\S]*/m, JSON.stringify(projectJSON, null, 4)))
                    .pipe(gulp.dest('./', {overwrite: true}).on('finish', () => {
                        cb()
                    }))
            })
        })
